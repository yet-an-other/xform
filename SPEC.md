...
CREATE TABLE roster (
  ...
  disabled   INTEGER NOT NULL DEFAULT 0,  -- off the Roster (ADR-0007); history kept
  deleting   INTEGER NOT NULL DEFAULT 0,  -- purge in progress (ADR-0007): apply, then erase
...
Deleting a user (ADR-0007, the second removal act) is two-phase: the row is marked `deleting` — disabled with it, so the dashboard renders it disabled while the removal applies and the usual Roster-sync surface carries pending/failed — then the removal applies exactly like a disable through the store → file render → API push machine, retrying on config-watch fires and xray running transitions; once it settles, the purge erases the roster row and the users row with its durable totals, last seen, and presence, releasing the email and its Client ID claim. The xray-wide totals are panel state, not the user's — untouched. The email and its Client ID stay claimed until the purge lands: a re-add of a deleting email is a conflict. Nothing keyed by the email remains after a purge, so nothing remembers it — a stale config or hand edit carrying a deleted client adopts it back as a **brand-new user** with fresh history (consistent with any foreign client; the provisioning contract keeps templates from carrying clients at all).
...
The User, Traffic, and Last seen headers SHALL sort the visible rows in the browser; sorting SHALL NOT change the API contract or server ordering. Before the first selection, the table SHALL preserve API order. Selecting User for the first time sorts case-insensitively by email, ascending; selecting Traffic or Last seen for the first time sorts descending. Selecting the active header toggles its direction. Selecting another sortable header uses that header's first-selection direction. Once sorting starts, there is no unsorted third state. The selection persists across reloads in the browser store, browser-local only: it never reaches the panel, another browser, or any other origin, and clearing site data resets it (ADR-0009).
...
`deploy/trusted-proxy/` contains explicit root/subpath examples: nginx and Caddy for embedded and static shapes, Traefik for the embedded shape, and oauth2-proxy with ZITADEL for both mounts. The ZITADEL example uses a Web application with the Code flow preset (client-secret BASIC) and an exact HTTPS callback; oauth2-proxy supplies issuer discovery, `openid email`, verified email, an exact-email allowlist, and PKCE S256 (`code_challenge_method`) — the console's PKCE preset is a public client without a secret and is not usable here. It disables identity and token forwarding. Panel sign-out clears only the oauth2-proxy session by default; broader ZITADEL logout is optional and documented separately.
