...
_Amendment (ADR-0008):_ the fixed invocation later gained a second compiled-in template for `filter=system`, adding one attached `--grep=` argument with a pinned PCRE2 negative lookahead (`^(?![0-9]{4}/[0-9]{2}/[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]+)? from )`) that drops xray Access records. `--grep` filters inside the journal traversal, so `--lines=500` still selects the newest 500 _matching_ records — a filtered snapshot reaches back past an access flood, which is the entire point; the byte caps, deadline, and classification rules above are untouched. `--invert` would read better but is absent on the deployed systemd (259), so the lookahead inverts instead; `--grep` requires journalctl built with PCRE2, pinned as a deployment requirement rather than probed at runtime. The argument-safety rule below is unchanged: the caller supplies an enum that selects the template, never the pattern. Measured on the live host, a filtered fetch scans a 40 MB namespaced journal in ~0.13 s.
