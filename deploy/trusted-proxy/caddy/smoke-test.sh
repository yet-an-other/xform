#!/bin/sh
# Run the Caddy Trusted proxy gateway contract against local deterministic
...
# Docker's Caddy worker must be able to read the bind-mounted fixture and
# socket directories. The fake socket is deliberately more permissive; real
# xform sockets remain 0660 in a protected group-owned directory.
chmod 0755 "$workdir"
...
render_config() {
    ...
    listen_port=$3
    secret=$4
    python3 - "$template" "$output" "$listen_port" "$auth_port" "$secret" "$static_root" <<'PY'
from pathlib import Path
import sys

template, output, listen_port, auth_port, secret, static_root = sys.argv[1:]
text = Path(template).read_text()
replacements = {
    "__CADDY_LISTEN__": f"http://:{listen_port}",
    "__OAUTH2_PROXY_UPSTREAM__": f"127.0.0.1:{auth_port}",
    "__XFORM_UPSTREAM__": "unix//run/xform/xform.sock",
    "__XFORM_TRUSTED_PROXY_SECRET__": secret,
    "__STATIC_ROOT__": "/var/www/xform/dist",
}
for marker, value in replacements.items():
    text = text.replace(marker, value)
if any(marker in text for marker in replacements):
    raise SystemExit("unexpanded Caddy template marker")
...
run_case() {
    ...
    [ -f "$template" ] || { printf 'missing Caddy template: %s\n' "$template" >&2; exit 1; }
    ...
    # The listener port is selected just before launch. If another local
    # process wins the small bind race, retry with a fresh port rather than
    # making the matrix flaky when two smoke runs overlap.
    start_attempt=0
    while [ "$start_attempt" -lt 5 ]; do
        listen_port=$(random_port)
        while [ "$auth_port" = "$listen_port" ]; do
            listen_port=$(random_port)
        done
        secret=$(python3 -c 'import secrets; print(secrets.token_hex(32))')
        caddy_config="$workdir/$case_name.Caddyfile"
        render_config "$template" "$caddy_config" "$listen_port" "$secret"
        ...
        while [ "$attempt" -lt 100 ]; do
            ...
            if ! kill -0 "$caddy_pid" 2>/dev/null; then
                ...
                caddy_pid=
                caddy_container=
                printf '%s Caddy exited\n' "$case_name" >&2
                cat "$workdir/$case_name.caddy.log" >&2
                exit 1
            fi
            attempt=$((attempt + 1))
            sleep 0.05
        done
        [ "$start_ready" -eq 1 ] && break
        [ "$retry_start" -eq 1 ] && continue
        printf '%s Caddy did not become ready\n' "$case_name" >&2
        cat "$workdir/$case_name.caddy.log" >&2
        exit 1
    done
    ...
    # A client-supplied rd query must not be reparsed as oauth2-proxy's
    # control query. The explicit empty-query rewrite makes the redirect
    # carry the complete original URI in the rd value instead.
    collision_target="$public_prefix/?rd=spoofed&return=%2F"
    if [ "$shape" = "root" ]; then
        expected_collision_rd='%2F%3Frd%3Dspoofed%26return%3D%252F'
    else
        expected_collision_rd='%2Fxform%2F%3Frd%3Dspoofed%26return%3D%252F'
    fi
    status=$(request document_rd_collision "$base$collision_target")
    [ "$status" = 302 ] || { printf '%s rd-collision document status=%s, want 302\n' "$case_name" "$status" >&2; exit 1; }
    grep -Fq "Location: $public_prefix/fake-idp?rd=$expected_collision_rd" "$workdir/$case_name.document_rd_collision.headers" || {
        printf '%s sign-in rewrite reparsed the original rd query\n' "$case_name" >&2
        cat "$workdir/$case_name.document_rd_collision.headers" >&2
        exit 1
    }
    assert_no_store "$workdir/$case_name.document_rd_collision.headers"
    ...
    status=$(request auth_check_public "$base$public_prefix/oauth2/auth")
    ...
    [ "$status" = 502 ] || { printf '%s document auth error status=%s, want fail-closed 502\n' "$case_name" "$status" >&2; exit 1; }

    status=$(request auth_error -H 'Cookie: xform_test_auth=error' "$base$api_target")
    [ "$status" = 502 ] || { printf '%s forward-auth error status=%s, want fail-closed 502\n' "$case_name" "$status" >&2; exit 1; }
...
for name in (
    ...
    "X-Forwarded-Authorization", "X-Forwarded-Client-Cert",
    "X-Auth-Request-User", "X-Auth-Request-Email", "X-Auth-Request-Groups",
    "X-Auth-Request-Preferred-Username", "X-Auth-Request-Token",
    "X-Auth-Request-Access-Token", "X-Auth-Request-Redirect",
    "X-Auth-Request-Id-Token", "X-Auth-Request-IdToken", "X-Access-Token",
    "X-ID-Token", "X-Id-Token", "Remote-User", "Remote-Email",
    "Remote-Groups", "X-Remote-User", "X-Remote-Email", "X-Remote-Groups",
    "X-Authenticated-User", "X-Authenticated-Email", "X-User", "X-Email",
    "X-Groups", "X-Group", "X-Original-URL", "X-Original-URI",
    "X-Original-Method", "X-SSL-Client-Cert", "X-Client-Cert", "X-Real-IP",
):
...
PY
    ...
    if [ "$template_name" = "root-embedded.Caddyfile.template" ] || [ "$template_name" = "subpath-embedded.Caddyfile.template" ]; then
        ...
        if [ "$shape" = "root" ]; then
            expected_embedded_target='/users/alice%2Fbob?query=%25%2F'
        else
            expected_embedded_target='/users/alice%2Fbob?query=%25%2F'
        fi
        grep -Fq "\"request_target\":\"$expected_embedded_target\"" "$workdir/$case_name.embedded_document.body" || {
            printf '%s embedded document did not reach the Panel with its encoded target\n' "$case_name" >&2
            exit 1
        }
    fi

    if [ "$template_name" = "root-static.Caddyfile.template" ] || [ "$template_name" = "subpath-static.Caddyfile.template" ]; then
        ...
        [ "$status" = 502 ] || { printf '%s asset auth error status=%s, want fail-closed 502\n' "$case_name" "$status" >&2; exit 1; }
        ...
        grep -Fq 'Cache-Control: public, max-age=31536000, immutable' "$workdir/$case_name.asset.headers" || {
            printf '%s asset did not receive immutable caching\n' "$case_name" >&2
            exit 1
        }
...
run_case root-embedded root-embedded.Caddyfile.template root
run_case subpath-embedded subpath-embedded.Caddyfile.template subpath
run_case root-static root-static.Caddyfile.template root
run_case subpath-static subpath-static.Caddyfile.template subpath

printf '%s\n' 'Caddy Trusted proxy smoke matrix: PASS'
